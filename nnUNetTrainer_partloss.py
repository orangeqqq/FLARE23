...
from nnunetv2.training.loss.part_losses import Part_DC_and_BCE_loss,PartMemoryEfficientSoftDiceLoss

class nnUNetTrainer_partloss(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.num_epochs = 300
        self.save_every = 10
        # self.label_manager.has_regions = True
        
        self.label_manager = self.plans_manager.get_label_manager(dataset_json, inference_nonlin = torch.sigmoid)


    def _build_loss(self):
        # 设置loss为 one-hot?
        # 包含背景，c+1
        loss = Part_DC_and_BCE_loss({},
